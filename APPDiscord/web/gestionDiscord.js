                ...
                webSocket.onmessage = function(event){
                    ...
                    else if(event.data==="authentification succeed")
                    {
                        ...
                        $.ajax({ //envoie d'une requete en ajax vers une servlet me permettant d'afficher les boutons avec les noms
                            ...
                            url: ADRESSE_SERVEUR+'APPDiscord/ServletAmis',
                            type: 'POST',
                    ...
                    else //partie gestion des réceptions des messages venant du serveur endpoint lorsqu'on se trouve dans une room ou dans une discussion avec une personne
                    {
                        ...
                        if(objString.includes("channel\":\"TI"))//si c'est un message qui a été envoyé à partir du room TI
                        {
                                ...
                                {
                                    if(objJSON.message.includes("youtube.com/watch?v="))//si dans le message contient un lien youtube, il m'affiche la vidéo dans une frame
                                    {
                                        var idVideo = objJSON.message.split("watch?v=")[1];//récupération de l'ID de la vidéo youtube pour pouvoir appliquer un embed dessus pour pouvoir l'afficher dans l'iframe
                                        if(objJSON.message.indexOf("&")!=-1){//si la vidéo a été récupéré dans une playlist
            ...
            window.onload =function(){//dès que la page web c'est affichée
                ...
                $(document).on('click','#btnChannelTI',function(){//si j'ai cliqué sur un bouton dans le menu pour parler à un ami
                    ...
                    $.ajax({ //il réactualise la liste des boutons qui se trouve dans la zone du menu, ami, pour afficher, si un nouvel utilisateur 
                            ...
                            url: ADRESSE_SERVEUR+'APPDiscord/ServletAmis',//permet de faire appel à un fichier JSP qui va afficher les boutons qui me permettront d'accéder à un 
                    ...
                    $.ajax({ //il m'affiche les messages qui ont déjà été postés dans le channel TI
                        url: ADRESSE_SERVEUR+'APPDiscord/ServletMessage',
                        type: 'POST',
                        data: { channel: "TI"} ,
                        contentType: 'application/json; charset=utf-8',
                        success: function (response) {
                ...
                $(document).on('click','#btnJeu',function(){//gestion du bouton pour afficher un jeu
                    ...
                    $("#EcrivezA").empty();
                     ...
                     {
                        $("#partieJeu").append("<iframe id=\"fenetreJeu\" width=\"800\" height=\"650\" src=\""+ADRESSE_SERVEUR+"APPDiscord/jquery-picture-memory-game/index.html\"></iframe>"); //affichage d'une iframe contenant un jeu en HTML5
                ...
                $(document).on('click','#btnRetourAuthenti',function(){//gestion du bouton pour retourner au formulaire d'authentification si l'ion se trouvait sur la page d'inscription
                     $('#unPseudo').val('');
                     $('#unMDP').val('');
                     $('#unPseudoNouvelleUtilisateur').val('');
                     $('#unPseudoNouvelleMDP').val('');
